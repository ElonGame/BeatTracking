...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i<(speclen/2 + 1); i++)
    {
        ...
        {
            fprintf(spectrogram, "%f ",spec->mag_spec[i*(data.maxFrameIndex/speclen)+j]);
        }
       fprintf(spectrogram,"\n");
    }
    ...
    for(int j = 0; j<(data.maxFrameIndex/(hop_size*2)); j++)
    {
        fprintf(novelty, "%f ",novelty_curve[j]);
    }
    fprintf(novelty,"\n");
    ...
    for (int i = 0; i<(speclen_tempo/2 + 1); i++)
    {
        ...
        {
            fprintf(tempogram, "%f ",spec_tempo->mag_spec[i*((data.maxFrameIndex/(hop_size*2))/speclen_tempo)+j]);
        }
       fprintf(tempogram,"\n");
    }
