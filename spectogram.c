...
static void get_colour_map_value (float value, SAMPLE spec_floor_db, unsigned char colour [3], bool gray_scale)
{
	static unsigned char map [][3] = {	/* These values were originally calculated for a dynamic range of 180dB. */
		{	255,	255,	255	},	/* -0dB */
		{	240,	254,	216	},	/* -10dB */
		{	242,	251,	185	},	/* -20dB */
		{	253,	245,	143	},	/* -30dB */
...
static void render_spectrogram (cairo_surface_t * surface, SAMPLE spec_floor_db, float **mag2d, SAMPLE maxval, SAMPLE left, SAMPLE top, SAMPLE width, SAMPLE height, bool gray_scale)
{
	unsigned char colour [3] = { 0, 0, 0 };
	unsigned char *data;
	SAMPLE linear_spec_floor;
	int w, h, stride;
...
static void render_heat_map (cairo_surface_t * surface, SAMPLE magfloor, const RECT *r, bool gray_scale)
{
...
static inline void x_line (cairo_t * cr, SAMPLE x, SAMPLE y, SAMPLE len)
{
...
static inline void y_line (cairo_t * cr, SAMPLE x, SAMPLE y, SAMPLE len)
{
	cairo_move_to (cr, x, y);
...
static int calculate_log_ticks (SAMPLE min, SAMPLE max, SAMPLE distance, TICKS * ticks);
...
static int calculate_ticks (SAMPLE min, SAMPLE max, SAMPLE distance, int log_scale, TICKS * ticks)
{
	SAMPLE step;	/* Put numbered ticks at multiples of this */
	SAMPLE range = max - min;
	int k;
	SAMPLE value;	/* Temporary */
	...
	* the last one that's <= max. */
	...
	{	\
		...
		{	\
			...
			ticks->distance [k] = distance * (log_scale == 2  ? /*log*/ (log (val) - log (min)) / (log (max) - log (min)) : /*lin*/ (val - min) / range); \
			k++; \
		}; \
	} while (0)
...
*/
static int add_log_ticks (SAMPLE min, SAMPLE max, SAMPLE distance, TICKS * ticks, int k, SAMPLE start_value, bool include_number)
{
	SAMPLE value;
...
static int calculate_log_ticks (SAMPLE min, SAMPLE max, SAMPLE distance, TICKS * ticks)
{
	...
	SAMPLE underpinning; 	/* Largest power of ten that is <= min */
...
static void str_print_value (char * text, int text_len, SAMPLE value, int decimal_places_to_print)
{
	if (fabs (value) < 1e-10)
...
static void render_spect_border (cairo_surface_t * surface, SAMPLE left, SAMPLE width, float seconds, SAMPLE top, SAMPLE height, SAMPLE min_freq, SAMPLE max_freq, bool log_freq)
{
	char text [512];
	cairo_t * cr;
...
static void render_heat_border (cairo_surface_t * surface, SAMPLE magfloor, const RECT *r)
{
...
*/
static SAMPLE magindex_to_specindex (int speclen, int maglen, int magindex, SAMPLE min_freq, SAMPLE max_freq, bool log_freq)
{
	SAMPLE freq; /* The frequency that this output value represents */

	if (!log_freq)
	freq = min_freq + (max_freq - min_freq) * magindex / (maglen - 1);
	else
	freq = min_freq * pow (max_freq / min_freq, (SAMPLE) magindex / (maglen - 1));
...
*/
static void interp_spec (float * mag, int maglen, const SAMPLE *spec, int speclen, const RENDER *render)
{
	int k;
	...
	*/
	...
	{	/* Average the pixels in the range it comes from */
		SAMPLE this = magindex_to_specindex (speclen, maglen, k, render->min_freq, render->max_freq, render->log_freq);
		SAMPLE next = magindex_to_specindex (speclen, maglen, k+1, render->min_freq, render->max_freq, render->log_freq);
		...
		{	/* The output indices are more sparse than the input indices
			...
			*/
			...
			SAMPLE count = 1.0 - (this - floor (this));
			SAMPLE sum = spec [(int) this] * count;
...
static void render_to_surface (const RENDER * render, cairo_surface_t * surface)
{
	...
	*/
	if (render->fft_freq != 0.0)
		...
		speclen = (SAMPLE_RATE / render->fft_freq + 1) / 2;
	else
		/* Long enough to represent frequencies down to 20Hz. */
		speclen = height * (SAMPLE_RATE / 20 / height + 1);
	...
	for (w = 0; w < width; w++)
	{
		...
		for (int i = 0; i< (2*speclen); i++)
			spec->time_domain[i] = render->data.recordedSamples[w*width + i];
	...
	if (render->border)
	{
		...
		render_spect_border (surface, LEFT_BORDER, width, render->data.maxFrameIndex/SAMPLE_RATE, TOP_BORDER, height, render->min_freq, render->max_freq, render->log_freq);
...
static void render_cairo_surface (const RENDER * render)
{
	...
	*/
	...
	render_to_surface (render, surface);
