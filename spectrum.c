...
spectrum * create_spectrum (int speclen, enum WINDOW_FUNCTION window_function)
{
	...
	*/
	spec->time_domain = calloc (2 * speclen + 1, sizeof (SAMPLE));
	spec->window = calloc (2 * speclen, sizeof (SAMPLE));
	spec->freq_domain = calloc (2 * speclen, sizeof (SAMPLE));
	spec->mag_spec = calloc (speclen + 1, sizeof (SAMPLE));
	...
	spec->plan = fftwf_plan_r2r_1d (2 * speclen, spec->time_domain, spec->freq_domain, FFTW_R2HC, FFTW_MEASURE | FFTW_PRESERVE_INPUT);
	if (spec->plan == NULL)
	{
...
SAMPLE calc_magnitude_spectrum (spectrum * spec)
{
	SAMPLE max;
	int k, freqlen;
	...
	**/
	...
	{
		SAMPLE re = spec->freq_domain [k];
		SAMPLE im = spec->freq_domain [freqlen - k];
